...
export class JewishCalendarSensors {
  ...
  constructor(platform: ZigbeePlatform, jewishCalendarConfig: JewishCalendarSensorsConfig) {
    ...
    const testSensorItems = [
      ...
      { label: 'Sukkot Yom Tov', mode: 7, semanticTags: [] },
      { label: 'Shemini Atzeret', mode: 8, semanticTags: [] },
      { label: 'Chanukah', mode: 9, semanticTags: [] },
      { label: 'Purim', mode: 10, semanticTags: [] },
      { label: 'Shushan Purim', mode: 11, semanticTags: [] },
      { label: 'Purim Meshulash', mode: 12, semanticTags: [] },
      { label: 'Pesach', mode: 13, semanticTags: [] },
      { label: 'Pesach 1st Yom Tov', mode: 14, semanticTags: [] },
      { label: 'Shvihi Shel Pesach', mode: 15, semanticTags: [] },
      { label: 'Shavuot', mode: 16, semanticTags: [] },
      { label: 'Three Weeks', mode: 17, semanticTags: [] },
      { label: 'Sefirat HaOmer Mourning', mode: 18, semanticTags: [] },
      { label: 'Sefirat HaOmer', mode: 19, semanticTags: [] },
      { label: 'Mourning', mode: 20, semanticTags: [] },
      { label: 'Leap Year', mode: 21, semanticTags: [] },
    ];
    if (this.config.showEves) {
      testSensorItems.push(
        { label: 'Shabbat Eve', mode: 22, semanticTags: [] },
        { label: 'Yom Tov Eve', mode: 23, semanticTags: [] },
        { label: 'Kodesh Eve', mode: 24, semanticTags: [] },
        { label: 'Rosh Hashana Eve', mode: 25, semanticTags: [] },
        { label: 'Yom Kippur Eve', mode: 26, semanticTags: [] },
        { label: 'Sukkot Eve', mode: 27, semanticTags: [] },
        { label: 'Shemini Atzeret Eve', mode: 28, semanticTags: [] },
        { label: 'Chanukah Eve', mode: 29, semanticTags: [] },
        { label: 'Purim Eve', mode: 30, semanticTags: [] },
        { label: 'Shushan Purim Eve', mode: 31, semanticTags: [] },
        { label: 'Purim Meshulash Eve', mode: 32, semanticTags: [] },
        { label: 'Pesach Eve', mode: 33, semanticTags: [] },
        { label: 'Shvihi Shel Pesach Eve', mode: 34, semanticTags: [] },
        { label: 'Shavuot Eve', mode: 35, semanticTags: [] },
      );
    }
    ...
    this.services.SukkotYomTov = new JewishCalendarSensor(this.sensor, { name: 'Sukkot Yom Tov', debug: platform.config.debug });
    ...
    this.services.Pesach1stYomTov = new JewishCalendarSensor(this.sensor, { name: 'Pesach 1st Yom Tov', debug: platform.config.debug });
  ...
  async updateSensors(): Promise<void> {
    ...
    const isShabbatNow = this.isShabbat();
    const isYomTovNow = this.isYomTov();
    const isKodeshNow = this.isKodesh();
    const isRoshHashanaNow = this.isRoshHashana();
    const isYomKippurNow = this.isYomKippur();
    const isSukkotNow = this.isSukkot();
    const isSheminiAtzeretNow = this.isSheminiAtzeret();
    const isChanukahNow = this.isChanukah();
    const isPurimNow = this.isPurim();
    const isShushanPurimNow = this.isShushanPurim();
    const isPurimMeshulashNow = this.isPurimMeshulash();
    const isPesachNow = this.isPesach();
    const isShvihiShelPesachNow = this.isShvihiShelPesach();
    const isShavuotNow = this.isShavuot();
    await this.services.Shabbat.update(isShabbatNow);
    await this.services.YomTov.update(isYomTovNow);
    await this.services.Kodesh.update(isKodeshNow);
    await this.services.RoshHashana.update(isRoshHashanaNow);
    await this.services.YomKippur.update(isYomKippurNow);
    await this.services.Sukkot.update(isSukkotNow);
    await this.services.SukkotYomTov.update(this.isSukkotYomTov());
    await this.services.SheminiAtzeret.update(isSheminiAtzeretNow);
    await this.services.Chanukah.update(isChanukahNow);
    await this.services.Purim.update(isPurimNow);
    await this.services.ShushanPurim.update(isShushanPurimNow);
    await this.services.PurimMeshulash.update(isPurimMeshulashNow);
    await this.services.Pesach.update(isPesachNow);
    await this.services.Pesach1stYomTov.update(this.isPesach1stYomTov());
    await this.services.ShvihiShelPesach.update(isShvihiShelPesachNow);
    await this.services.Shavuot.update(isShavuotNow);
    ...
    if (this.config.showEves) {
      await this.services.ShabbatEve.update(!isShabbatNow && this.isShabbatEve());
      await this.services.YomTovEve.update(!isYomTovNow && this.isYomTovEve());
      await this.services.KodeshEve.update(!isKodeshNow && this.isKodeshEve());
      await this.services.RoshHashanaEve.update(!isRoshHashanaNow && this.isRoshHashanaEve());
      await this.services.YomKippurEve.update(!isYomKippurNow && this.isYomKippurEve());
      await this.services.SukkotEve.update(!isSukkotNow && this.isSukkotEve());
      await this.services.SheminiAtzeretEve.update(!isSheminiAtzeretNow && this.isSheminiAtzeretEve());
      await this.services.ChanukahEve.update(!isChanukahNow && this.isChanukahEve());
      await this.services.PurimEve.update(!isPurimNow && this.isPurimEve());
      await this.services.ShushanPurimEve.update(!isShushanPurimNow && this.isShushanPurimEve());
      await this.services.PurimMeshulashEve.update(!isPurimMeshulashNow && this.isPurimMeshulashEve());
      await this.services.PesachEve.update(!isPesachNow && this.isPesachEve());
      await this.services.ShvihiShelPesachEve.update(!isShvihiShelPesachNow && this.isShvihiShelPesachEve());
      await this.services.ShavuotEve.update(!isShavuotNow && this.isShavuotEve());
    }
  }
  ...
  isRoshHashanaEve(): boolean {
    if (this.hDate.getMonth() == this.hebrewMonths.Elul) {
      if (this.hDate.getDate() == 28) {
        const candletime = new Date(this.sunset);
        candletime.setMinutes(this.sunset.getMinutes() - this.config.candlelighting);
        return this.gDate > candletime;
      } else if (this.hDate.getDate() == 29 && !this.isRoshHashana()) {
        return true;
      }
    }
    return false;
