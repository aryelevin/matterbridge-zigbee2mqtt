...
export class ZigbeeEntity extends EventEmitter {
  ...
  constructor(platform: ZigbeePlatform, entity: BridgeDevice | BridgeGroup) {
    ...
    this.platform.z2m.on('MESSAGE-' + this.entityName, (incomingPayload: Payload) => {
      // Changed by me: Arye Levin (from payload to incomingPayload)
      // Added by me: Arye Levin
      ...
      if (this.noUpdate) {
        // TODO: If i change a device state from matter side and right away i change it again from the device side, the state is not refelected on the matter side (Including switches off mode which will miss this change as will not enforce the state), I need a solution for this...
  ...
  // prettier-ignore
  protected  onCommandHandler(data: CommandHandlerData) {
    ...
    if (this.isDevice && this.device && !this.propertyMap.get('state' + (isChildEndpoint ? endpoint : '')) && this.propertyMap.get('system_mode' + (isChildEndpoint ? endpoint : ''))?.type === 'ac') {
      const value = data.endpoint.getAttribute(Thermostat.id, 'systemMode');
  ...
  // prettier-ignore
  protected  offCommandHandler(data: CommandHandlerData) {
  ...
  // prettier-ignore
  protected  toggleCommandHandler(data: CommandHandlerData) {
  ...
  // prettier-ignore
  protected  moveToLevelCommandHandler(data: CommandHandlerDataMap['LevelControl.moveToLevel']) {
  ...
  // prettier-ignore
  protected  moveToLevelWithOnOffCommandHandler(data: CommandHandlerDataMap['LevelControl.moveToLevelWithOnOff']) {
    ...
    if (data.request['level'] <= (data.endpoint.getAttribute(LevelControl.id, 'minLevel') ?? 1)) {
      ...
      if (data.endpoint.getAttribute(OnOff.id, 'onOff') === false) {
        ...
        // Added by me: Arye Levin
        const shouldTurnOn = this.platform.switchingController.deviceHasChangedMatterAttribute(
          this.isDevice && this.device?.ieee_address ? this.device?.ieee_address : this.isGroup && this.group?.friendly_name ? this.group?.friendly_name : '',
          isChildEndpoint ? endpoint : '',
          'onOff',
          true,
  ...
  // prettier-ignore
  protected  moveToColorTemperatureCommandHandler(data: CommandHandlerDataMap['ColorControl.moveToColorTemperature']) {
    ...
    if (this.propertyMap.get('color_temp')) {
      this.cachePublish('moveToColorTemperature', { ['color_temp' + (isChildEndpoint ? endpoint : '')]: data.request.colorTemperatureMireds }, data.request.transitionTime);
    } else {
      const rgb = kelvinToRGB(miredToKelvin(data.request.colorTemperatureMireds)); // Convert mireds to RGB
      this.cachePublish('moveToColorTemperature', { ['color' + (isChildEndpoint ? endpoint : '')]: { r: rgb.r, g: rgb.g, b: rgb.b } }, data.request.transitionTime);
  ...
  // prettier-ignore
  protected  moveToColorCommandHandler(data: CommandHandlerDataMap['ColorControl.moveToColor']) {
    ...
    const isChildEndpoint = data.endpoint.deviceName !== this.entityName;
    const endpoint = isChildEndpoint ? ('_' + (data.endpoint.id.split('_')[1] || data.endpoint.id)) : ''; // For a separate endpoint of a device
    this.cachePublish('moveToColor', { ['color' + (isChildEndpoint ? endpoint : '')]: { x: Math.round(data.request.colorX / 65536 * 10000) / 10000, y: Math.round(data.request.colorY / 65536 * 10000) / 10000 } }, data.request.transitionTime);
  }

  // prettier-ignore
  protected  moveToHueCommandHandler(data: CommandHandlerDataMap['ColorControl.moveToHue']) {
    ...
    const isChildEndpoint = data.endpoint.deviceName !== this.entityName;
    const endpoint = isChildEndpoint ? ('_' + (data.endpoint.id.split('_')[1] || data.endpoint.id)) : ''; // For a separate endpoint of a device
    this.cachePublish('moveToHue', { ['color' + (isChildEndpoint ? endpoint : '')]: { h: Math.round(data.request.hue / 254 * 360), s: Math.round(data.endpoint.getAttribute(ColorControl.id, 'currentSaturation') / 254 * 100) }}, data.request.transitionTime);
  }

  // prettier-ignore
  protected  moveToSaturationCommandHandler(data: CommandHandlerDataMap['ColorControl.moveToSaturation']) {
    ...
    const isChildEndpoint = data.endpoint.deviceName !== this.entityName;
    const endpoint = isChildEndpoint ? ('_' + (data.endpoint.id.split('_')[1] || data.endpoint.id)) : ''; // For a separate endpoint of a device
    this.cachePublish('moveToSaturation', { ['color' + (isChildEndpoint ? endpoint : '')]: { h: Math.round(data.endpoint.getAttribute(ColorControl.id, 'currentHue') / 254 * 360), s: Math.round(data.request.saturation / 254 * 100) } }, data.request.transitionTime);
  }

  // prettier-ignore
  protected  moveToHueAndSaturationCommandHandler(data: CommandHandlerDataMap['ColorControl.moveToHueAndSaturation']) {
    ...
    const isChildEndpoint = data.endpoint.deviceName !== this.entityName;
    const endpoint = isChildEndpoint ? ('_' + (data.endpoint.id.split('_')[1] || data.endpoint.id)) : ''; // For a separate endpoint of a device
    this.cachePublish('moveToHueAndSaturation', { ['color' + (isChildEndpoint ? endpoint : '')]: { h: Math.round(data.request.hue / 254 * 360), s: Math.round(data.request.saturation / 254 * 100) } }, data.request.transitionTime);
  }
...
const z2ms: ZigbeeToMatter[] = [
  { type: 'switch', name: 'state', property: 'state', deviceType: onOffSwitch, cluster: OnOff.id, attribute: 'onOff', converter: (value) => { return value === 'ON' ? true : false } },
  { type: 'switch', name: 'brightness', property: 'brightness', deviceType: dimmableSwitch, cluster: LevelControl.id, attribute: 'currentLevel', converter: (value) => { return Math.max(1, Math.min(254, value)) } },
  { type: 'switch', name: 'color_hs', property: 'color_hs', deviceType: colorTemperatureSwitch, cluster: ColorControl.id, attribute: 'colorMode' },
  { type: 'switch', name: 'color_xy', property: 'color_xy', deviceType: colorTemperatureSwitch, cluster: ColorControl.id, attribute: 'colorMode' },
  { type: 'switch', name: 'color_temp', property: 'color_temp', deviceType: colorTemperatureSwitch, cluster: ColorControl.id, attribute: 'colorMode' },
  { type: 'outlet', name: 'state', property: 'state', deviceType: onOffOutlet, cluster: OnOff.id, attribute: 'onOff', converter: (value) => { return value === 'ON' ? true : false } },
  { type: 'outlet', name: 'brightness', property: 'brightness', deviceType: dimmableOutlet, cluster: LevelControl.id, attribute: 'currentLevel', converter: (value) => { return Math.max(1, Math.min(254, value)) } },
  { type: 'light', name: 'state', property: 'state', deviceType: onOffLight, cluster: OnOff.id, attribute: 'onOff', converter: (value) => { return value === 'ON' ? true : false } },
  ...
  { type: 'cover', name: 'state', property: 'state', deviceType: coverDevice, cluster: WindowCovering.id, attribute: 'targetPositionLiftPercent100ths' /* , converter: (value) => {return value === 'OPEN' ? 0 : value === 'CLOSE' ? 10000 : 5000}*/ },
  { type: 'cover', name: 'moving', property: 'moving', deviceType: coverDevice, cluster: WindowCovering.id, attribute: 'operationalStatus' },
  { type: 'cover', name: 'motor_state', property: 'motor_state', deviceType: coverDevice, cluster: WindowCovering.id, attribute: 'operationalStatus' },
  { type: 'cover', name: 'position', property: 'position', deviceType: coverDevice, cluster: WindowCovering.id, attribute: 'currentPositionLiftPercent100ths' },
  { type: 'lock', name: 'state', property: 'state', deviceType: doorLockDevice, cluster: DoorLock.id, attribute: 'lockState', converter: (value) => { return value === 'LOCK' ? DoorLock.LockState.Locked : DoorLock.LockState.Unlocked } },
  { type: 'climate', name: 'local_temperature', property: 'local_temperature', deviceType: thermostatDevice, cluster: Thermostat.id, attribute: 'localTemperature', converter: (value) => { return Math.max(-5000, Math.min(5000, value * 100)) } },
  { type: 'climate', name: 'current_heating_setpoint', property: 'current_heating_setpoint', deviceType: thermostatDevice, cluster: Thermostat.id, attribute: 'occupiedHeatingSetpoint' },
  { type: 'climate', name: 'occupied_heating_setpoint', property: 'occupied_heating_setpoint', deviceType: thermostatDevice, cluster: Thermostat.id, attribute: 'occupiedHeatingSetpoint', converter: (value) => { return Math.max(-5000, Math.min(5000, value * 100)) } },
  { type: 'climate', name: 'occupied_cooling_setpoint', property: 'occupied_cooling_setpoint', deviceType: thermostatDevice, cluster: Thermostat.id, attribute: 'occupiedCoolingSetpoint', converter: (value) => { return Math.max(-5000, Math.min(5000, value * 100)) } },
  { type: 'climate', name: 'unoccupied_heating_setpoint', property: 'unoccupied_heating_setpoint', deviceType: thermostatDevice, cluster: Thermostat.id, attribute: 'occupiedHeatingSetpoint', converter: (value) => { return Math.max(-5000, Math.min(5000, value * 100)) } },
  { type: 'climate', name: 'unoccupied_cooling_setpoint', property: 'unoccupied_cooling_setpoint', deviceType: thermostatDevice, cluster: Thermostat.id, attribute: 'occupiedCoolingSetpoint', converter: (value) => { return Math.max(-5000, Math.min(5000, value * 100)) } },
  { type: 'climate', name: 'running_state', property: 'running_state', deviceType: thermostatDevice, cluster: Thermostat.id, attribute: 'thermostatRunningMode', valueLookup: ['idle', '', '', 'cool', 'heat'] },
  { type: 'climate', name: 'system_mode', property: 'system_mode', deviceType: thermostatDevice, cluster: Thermostat.id, attribute: 'systemMode', valueLookup: ['off', 'auto', '', 'cool', 'heat', '', '', 'fan_only'] },
  { type: 'ac', name: 'state', property: 'state', deviceType: airConditioner, cluster: OnOff.id, attribute: 'onOff', converter: (value) => { return value === 'ON' ? true : false } },
  { type: 'ac', name: 'local_temperature', property: 'local_temperature', deviceType: airConditioner, cluster: Thermostat.id, attribute: 'localTemperature', converter: (value) => { return Math.max(-5000, Math.min(5000, value * 100)) } },
  { type: 'ac', name: 'occupied_heating_setpoint', property: 'occupied_heating_setpoint', deviceType: airConditioner, cluster: Thermostat.id, attribute: 'occupiedHeatingSetpoint', converter: (value) => { return Math.max(-5000, Math.min(5000, value * 100)) } },
  { type: 'ac', name: 'occupied_cooling_setpoint', property: 'occupied_cooling_setpoint', deviceType: airConditioner, cluster: Thermostat.id, attribute: 'occupiedCoolingSetpoint', converter: (value) => { return Math.max(-5000, Math.min(5000, value * 100)) } },
  { type: 'ac', name: 'system_mode', property: 'system_mode', deviceType: airConditioner, cluster: Thermostat.id, attribute: 'systemMode', valueLookup: ['', 'auto', '', 'cool', 'heat', '', '', 'fan_only'] },
  { type: 'ac', name: 'fan_mode', property: 'fan_mode', deviceType: airConditioner, cluster: FanControl.id, attribute: 'fanMode', valueLookup: ['off', 'low', 'medium', 'high', 'on', 'auto', 'smart'] },
  { type: 'fan', name: 'mode', property: 'fan_mode', deviceType: fanDevice, cluster: FanControl.id, attribute: 'fanMode', valueLookup: ['off', 'low', 'medium', 'high', 'on', 'auto', 'smart'] },
  { type: '', name: 'min_temperature_limit', property: 'min_temperature_limit', deviceType: thermostatDevice, cluster: Thermostat.id, attribute: 'minHeatSetpointLimit', converter: (value) => { return Math.max(-5000, Math.min(5000, value * 100)) } },
  { type: '', name: 'max_temperature_limit', property: 'max_temperature_limit', deviceType: thermostatDevice, cluster: Thermostat.id, attribute: 'maxHeatSetpointLimit', converter: (value) => { return Math.max(-5000, Math.min(5000, value * 100)) } },
  { type: '', name: 'min_heat_setpoint_limit', property: 'min_heat_setpoint_limit', deviceType: thermostatDevice, cluster: Thermostat.id, attribute: 'minHeatSetpointLimit', converter: (value) => { return Math.max(-5000, Math.min(5000, value * 100)) } },
  { type: '', name: 'max_heat_setpoint_limit', property: 'max_heat_setpoint_limit', deviceType: thermostatDevice, cluster: Thermostat.id, attribute: 'maxHeatSetpointLimit', converter: (value) => { return Math.max(-5000, Math.min(5000, value * 100)) } },
    ...
    */
    ...
    for (const [index, name] of names.entries()) {
      ...
      if (z2m) {
        ...
        const endpointToConsider = hasOneOnlyEndpoint ? '' : endpoint;
        if (endpointToConsider === '') {
          /* prettier-ignore */
          if (!zigbeeDevice.mutableDevice.has(endpointToConsider)) { zigbeeDevice.mutableDevice.set(endpointToConsider, { tagList: [], deviceTypes: [z2m.deviceType], clusterServersIds: [...z2m.deviceType.requiredServerClusters, ClusterId(z2m.cluster)], clusterServersOptions: [], clusterClientsIds: [], clusterClientsOptions: [] });
          } else {
            zigbeeDevice.mutableDevice.get(endpointToConsider)?.deviceTypes.push(z2m.deviceType);
            zigbeeDevice.mutableDevice.get(endpointToConsider)?.clusterServersIds.push(...z2m.deviceType.requiredServerClusters);
            zigbeeDevice.mutableDevice.get(endpointToConsider)?.clusterServersIds.push(ClusterId(z2m.cluster));
          }
        } else {
      ...
      if (name === 'action' && zigbeeDevice.actions.length) {
        ...
        for (const buttonName in switchesActions) {
          ...
          for (let index = 0; index < buttonActions.length; index++) {
            ...
            zigbeeDevice.propertyMap.set('action_' + action.actionItem, {
              name,
              type: '',
              endpoint: 'switch_' + buttonActionsData.switchNo,
              action: supportedActions[action.actionName],
            });
    ...
    const endpointSuffix = hasOneOnlyEndpoint ? '_' + theOneOnlyEndpoint : '';
    ...
    zigbeeDevice.bridgedDevice = new MatterbridgeEndpoint(
      ...
      { id: device.friendly_name + endpointSuffix },
    ...
    if (mainEndpoint.clusterServersIds.includes(Thermostat.id)) {
      ...
      const cool =
        zigbeeDevice.propertyMap.get('occupied_cooling_setpoint' + endpointSuffix) ||
        zigbeeDevice.propertyMap.get('unoccupied_cooling_setpoint' + endpointSuffix) /* Added by me: Arye Levin -> */ ||
        heat; // <- This is for cases where the device uses one zigbee attribute for both heating and cooling (Aqara W100)
      ...
      if ((heat && !cool) || (!system_mode_values?.includes('auto') && system_mode_values?.includes('heat'))) {
        zigbeeDevice.propertyMap.delete('running_state' + endpointSuffix); // Remove running_state if only heating is supported cause it's not supported by the cluster without AutoMode
        ...
        zigbeeDevice.propertyMap.delete('running_state' + endpointSuffix); // Remove running_state if only cooling is supported cause it's not supported by the cluster without AutoMode
        ...
        zigbeeDevice.bridgedDevice.createDefaultThermostatClusterServer(
          undefined,
          undefined,
          undefined,
          system_mode?.type === 'ac' ? 0 : undefined, // Allow cool and heat temps to be the same for ACs, since it independed and in many cases having one temperature setting for both (Aqara VRF Controller for example).
          minHeating,
          maxHeating,
          minCooling,
          maxCooling,
    ...
    if (zigbeeDevice.bridgedDevice.hasClusterServer(Thermostat.id)) {
      zigbeeDevice.bridgedDevice.addCommandHandler('setpointRaiseLower', ({ request: request }) => {
        ...
        if (request.mode === Thermostat.SetpointRaiseLowerMode.Heat || request.mode === Thermostat.SetpointRaiseLowerMode.Both) {
          ...
          if (zigbeeDevice.propertyMap.has('current_heating_setpoint' + endpointSuffix)) {
            zigbeeDevice.publishCommand('CurrentHeatingSetpoint', device.friendly_name, { ['current_heating_setpoint' + endpointSuffix]: setpoint });
          } else if (zigbeeDevice.propertyMap.has('occupied_heating_setpoint' + endpointSuffix)) {
            zigbeeDevice.publishCommand('OccupiedHeatingSetpoint', device.friendly_name, { ['occupied_heating_setpoint' + endpointSuffix]: setpoint });
          }
        }
        if (request.mode === Thermostat.SetpointRaiseLowerMode.Cool || request.mode === Thermostat.SetpointRaiseLowerMode.Both) {
          ...
          const setpoint = Math.round(t / 100 + request.amount / 10);
          if (zigbeeDevice.propertyMap.has('current_heating_setpoint' + endpointSuffix)) {
            zigbeeDevice.publishCommand('CurrentCoolingSetpoint', device.friendly_name, { ['current_heating_setpoint' + endpointSuffix]: setpoint });
          } else if (zigbeeDevice.propertyMap.has('occupied_cooling_setpoint' + endpointSuffix)) {
            zigbeeDevice.publishCommand('OccupiedCoolingSetpoint', device.friendly_name, { ['occupied_cooling_setpoint' + endpointSuffix]: setpoint });
          }
        }
      });
      void zigbeeDevice.bridgedDevice.subscribeAttribute(
        ...
        (newValue, oldValue, context) => {
          ...
          if (isValidNumber(newValue, Thermostat.SystemMode.Off, Thermostat.SystemMode.FanOnly) && zigbeeDevice.thermostatSystemModeLookup[newValue] !== '') {
            ...
            zigbeeDevice.publishCommand('SystemMode', device.friendly_name, { ['system_mode' + endpointSuffix]: system_mode });
      ...
      if (zigbeeDevice.bridgedDevice.hasAttributeServer(Thermostat.id, 'occupiedHeatingSetpoint'))
        void zigbeeDevice.bridgedDevice.subscribeAttribute(
          ...
          (newValue, oldValue, context) => {
            ...
            if (zigbeeDevice.propertyMap.has('current_heating_setpoint' + endpointSuffix))
              zigbeeDevice.publishCommand('OccupiedHeatingSetpoint', device.friendly_name, { ['current_heating_setpoint' + endpointSuffix]: Math.round(newValue / 100) });
            else if (zigbeeDevice.propertyMap.has('occupied_heating_setpoint' + endpointSuffix))
              zigbeeDevice.publishCommand('OccupiedHeatingSetpoint', device.friendly_name, { ['occupied_heating_setpoint' + endpointSuffix]: Math.round(newValue / 100) });
            // Added by me: Arye Levin
            if (
              !zigbeeDevice.propertyMap.has('current_cooling_setpoint' + endpointSuffix) &&
              !zigbeeDevice.propertyMap.has('occupied_cooling_setpoint' + endpointSuffix) &&
              zigbeeDevice.propertyMap
              ...
              zigbeeDevice.bridgedDevice?.setAttribute(Thermostat.id, 'occupiedCoolingSetpoint', newValue, zigbeeDevice.log);
            // End of Added by me: Arye Levin
            zigbeeDevice.noUpdate = true;
            zigbeeDevice.thermostatTimeout = setTimeout(() => {
              zigbeeDevice.noUpdate = false;
      ...
      if (zigbeeDevice.bridgedDevice.hasAttributeServer(Thermostat.id, 'occupiedCoolingSetpoint'))
        void zigbeeDevice.bridgedDevice.subscribeAttribute(
          ...
          (newValue, oldValue, context) => {
            ...
            if (zigbeeDevice.propertyMap.has('current_cooling_setpoint' + endpointSuffix))
              zigbeeDevice.publishCommand('OccupiedCoolingSetpoint', device.friendly_name, { ['current_cooling_setpoint' + endpointSuffix]: Math.round(newValue / 100) });
            else if (zigbeeDevice.propertyMap.has('occupied_cooling_setpoint' + endpointSuffix))
              zigbeeDevice.publishCommand('OccupiedCoolingSetpoint', device.friendly_name, { ['occupied_cooling_setpoint' + endpointSuffix]: Math.round(newValue / 100) });
            // Added by me: Arye Levin
            else if (
              !zigbeeDevice.propertyMap.has('current_cooling_setpoint' + endpointSuffix) &&
              !zigbeeDevice.propertyMap.has('occupied_cooling_setpoint' + endpointSuffix) &&
              zigbeeDevice.propertyMap.get('system_mode' + endpointSuffix)?.values?.includes('cool')
            )
              if (zigbeeDevice.propertyMap.has('current_heating_setpoint' + endpointSuffix))
                zigbeeDevice.publishCommand('OccupiedHeatingSetpoint', device.friendly_name, { ['current_heating_setpoint' + endpointSuffix]: Math.round(newValue / 100) });
              else if (zigbeeDevice.propertyMap.has('occupied_heating_setpoint' + endpointSuffix))
        ...
        void zigbeeDevice.bridgedDevice.subscribeAttribute(
          ...
          (newValue: FanControl.FanMode, oldValue: FanControl.FanMode, context) => {
            ...
            if (newValue === FanControl.FanMode.Off) {
              // zigbeeDevice.bridgedDevice?.setAttribute(FanControl.id, 'percentSetting', 0, zigbeeDevice.log);
              // zigbeeDevice.bridgedDevice?.setAttribute(FanControl.id, 'percentCurrent', 0, zigbeeDevice.log);
            } else if (newValue === FanControl.FanMode.Low) {
              zigbeeDevice.bridgedDevice?.setAttribute(FanControl.id, 'percentSetting', 33, zigbeeDevice.log);
              zigbeeDevice.bridgedDevice?.setAttribute(FanControl.id, 'percentCurrent', 33, zigbeeDevice.log);
            } else if (newValue === FanControl.FanMode.Medium) {
              zigbeeDevice.bridgedDevice?.setAttribute(FanControl.id, 'percentSetting', 66, zigbeeDevice.log);
              zigbeeDevice.bridgedDevice?.setAttribute(FanControl.id, 'percentCurrent', 66, zigbeeDevice.log);
            } else if (newValue === FanControl.FanMode.High) {
              zigbeeDevice.bridgedDevice?.setAttribute(FanControl.id, 'percentSetting', 100, zigbeeDevice.log);
              zigbeeDevice.bridgedDevice?.setAttribute(FanControl.id, 'percentCurrent', 100, zigbeeDevice.log);
            } else if (newValue === FanControl.FanMode.Auto) {
              zigbeeDevice.bridgedDevice?.setAttribute(FanControl.id, 'percentSetting', 10, zigbeeDevice.log);
              zigbeeDevice.bridgedDevice?.setAttribute(FanControl.id, 'percentCurrent', 10, zigbeeDevice.log);
            }
          },
          zigbeeDevice.log,
        );

      if (zigbeeDevice.bridgedDevice.hasAttributeServer(FanControl.id, 'percentSetting'))
        void zigbeeDevice.bridgedDevice.subscribeAttribute(
          FanControl.id,
          'percentSetting',
          (newValue: number | null, oldValue: number | null, context) => {
            ...
            if (isValidNumber(newValue, 0, 100)) {
              ...
              process.nextTick(() => {
                ...
                zigbeeDevice.bridgedDevice?.setAttribute(FanControl.id, 'percentSetting', fixedValue, zigbeeDevice.log);
                zigbeeDevice.bridgedDevice?.setAttribute(FanControl.id, 'percentCurrent', fixedValue, zigbeeDevice.log);
                ...
                zigbeeDevice.bridgedDevice?.setAttribute(FanControl.id, 'fanMode', fanModeSetting, zigbeeDevice.log);
              });
            }
          },
          zigbeeDevice.log,
