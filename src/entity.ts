      ...
      Object.entries(payload).forEach(([key, value]) => {
        ...
        if (key === 'action' && typeof value === 'string' && value !== '') {
          ...
          if (propertyMap) {
            ...
            // Added by me: Arye Levin
            if (value?.startsWith('brightness_step_')) {
              const newAction = value.replace('brightness_step_', '');
              const propertyMap1 = this.propertyMap.get('action_' + newAction);
              if (propertyMap1) {
...
const z2ms: ZigbeeToMatter[] = [
  ...
  { type: 'cover', name: 'motor_state', property: 'motor_state', deviceType: windowCovering, cluster: WindowCovering.id, attribute: 'operationalStatus' },
  ...
  { type: 'ac', name: 'system_mode', property: 'system_mode', deviceType: roomAirConditioner, cluster: Thermostat.id, attribute: 'systemMode', valueLookup: ['', 'auto', '', 'cool', 'heat', '', '', 'fan_only'] },
  { type: 'ac', name: 'fan_mode', property: 'fan_mode', deviceType: roomAirConditioner, cluster: FanControl.id, attribute: 'fanMode', valueLookup: ['off', 'low', 'medium', 'high', 'on', 'auto', 'smart'] },
  { type: 'fan', name: 'mode', property: 'fan_mode', deviceType: fan, cluster: FanControl.id, attribute: 'fanMode', valueLookup: ['off', 'low', 'medium', 'high', 'on', 'auto', 'smart'] },
    ...
    for (const [index, name] of names.entries()) {
      ...
      if (z2m) {
        ...
        if (endpointToConsider === '') {
          /* prettier-ignore */
          if (zigbeeDevice.mutableDevice.has(endpointToConsider)) {
          ...
          } else { zigbeeDevice.mutableDevice.set(endpointToConsider, { tagList: [], deviceTypes: [z2m.deviceType], clusterServersIds: [...z2m.deviceType.requiredServerClusters, ClusterId(z2m.cluster)], clusterServersOptions: [], clusterClientsIds: [], clusterClientsOptions: [] });
          }
        } else {
