...
export class ZigbeePlatform extends MatterbridgeDynamicPlatform {
  ...
  constructor(
    ...
    this.log.info(`Loaded zigbee2mqtt parameters from ${CYAN}${path.join(matterbridge.matterbridgeDirectory, 'matterbridge-zigbee2mqtt-al.config.json')}${rs}`);
    ...
    this.z2m.setDataPath(path.join(matterbridge.matterbridgePluginDirectory, 'matterbridge-zigbee2mqtt-al'));
        ...
        (async () => {
          ...
          if (this.shouldStart) {
            if (!this.z2mDevicesRegistered && this.z2mBridgeDevices) {
              const registeredDevicesMap: { [key: string]: BridgeDevice } = {};
              ...
              for (const key in this.devicesCache) {
                if (!registeredDevicesMap[key]) {
                  const device = this.devicesCache[key];
                  if (this.validateDevice([device.friendly_name, device.ieee_address], true)) {
                    ...
                    if (zigbeeDevice?.bridgedDevice) {
                      await zigbeeDevice.bridgedDevice.setAttribute(BridgedDeviceBasicInformation.id, 'reachable', false, this.log);
                      await zigbeeDevice.bridgedDevice.triggerEvent(BridgedDeviceBasicInformation.id, 'reachableChanged', { reachableNewValue: false }, this.log);
                    }
                  }
                }
              }
              this.devicesCache = registeredDevicesMap;
  ...
  public publish(topic: string, subTopic: string, message: string) {
  ...
  private requestDeviceUpdate(device: BridgeDevice) {
  ...
  private requestGroupUpdate(group: BridgeGroup) {
