    ...
    if (linkedDevices?.length) {
      ...
      if (!linkLastExecutionTime || Date.now() - linkLastExecutionTime >= 2000) {
        ...
        for (const linkedDevice of linkedDevices) {
          ...
          // this.log.info('deviceIeee: ' + deviceIeee  + ', linkedDeviceIeee: ' + linkedDeviceIeee + ', lastStates[linkedDeviceIeee]: ' + JSON.stringify(this.lastStates[linkedDeviceIeee]) + ', newPayload[paramToControl]: ' + JSON.stringify(newPayload[paramToControl]) + ', value: ' + value + '.');
